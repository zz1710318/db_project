...
if (!empty($search)) {
  $stmt = $link->prepare("SELECT * FROM `product` WHERE `name` LIKE :search OR `type` LIKE :search LIMIT :limit OFFSET :offset");
...
$stmt->execute();
    ...
    if (!empty($search)) {
      $stmt_count = $link->prepare("SELECT COUNT(*) as total FROM `product` WHERE `name` LIKE :search OR `type` LIKE :search");
...
if ($total_pages > 1) {
    ...
    $pagination_html .= '<li>';
    if ($page > 1) {
        // Include search query parameter in previous page link
        $pagination_html .= '<a href="?page=' . $prev_page . '&search=' . urlencode($search) . '" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>';
    } else {
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        ...
        // Include search query parameter in page links
        $pagination_html .= '><a href="?page=' . $i . '&search=' . urlencode($search) . '">' . $i . '</a></li>';
    }
    $pagination_html .= '<li>';
    if ($page < $total_pages) {
        // Include search query parameter in next page link
        $pagination_html .= '<a href="?page=' . $next_page . '&search=' . urlencode($search) . '" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>';
    } else {
...
if (empty($html)) {
  $not_found_message = '<div style="text-align: center;"><img src="https://deo.shopeemobile.com/shopee/shopee-pcmall-live-sg/search/a60759ad1dabe909c46a.png" alt="Not Found Image" style="width: 130px; height: 130px;"><h3 style="font-weight: bold; font-size: 1.2em;">抱歉，找不到您所查詢的「' . htmlspecialchars($search) . '」相關商品</h3><p style="font-size: 1.2em;">建議您，檢查輸入字詞是否有誤或使用其他相關的字詞再搜尋</p></div>';
...
<?php
    if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['addToCart'])) {
        ...
        echo '<script>window.location.href="productall.php";</script>';
    }
?>
    <?php
 ...
 <!-- SCROLL TOP BUTTON -->
  ...
  <header id="aa-header">
    ...
    <form action="productall.php" method="GET">
        <input type="text" name="search" placeholder="Search here ex. 'T-shirts'" value="<?php echo htmlspecialchars($search); ?>">
     ...
     <div class="container">
      <div class="aa-catg-head-banner-content">
        ...
        <ol class="breadcrumb">
          ...
          <li class="active">Short Sleeves</li>
        </ol>
      </div>
     </div>
   </div>
